...
public class FreezerRecipeSeria extends AbstractRecipeSerializer implements RecipeSerializer<FreezerRecipe> {
	...
	public FreezerRecipe fromJson(ResourceLocation id, JsonObject json) {
		...
		return new FreezerRecipe(id, resultList, ingredList, countList);
	}

	// サーバー側から受け取ったパケットから、レシピを復元する
	@Override
	public @Nullable FreezerRecipe fromNetwork(ResourceLocation id, FriendlyByteBuf buffer) {
	...
	@Override
	public void toNetwork(FriendlyByteBuf buffer, FreezerRecipe recipe) {
