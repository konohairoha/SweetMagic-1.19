...
public class RenderTridentThunder<T extends TridentThunder> extends RenderMagicBase<T> {
	...
	public void render(T entity, float yaw, float part, PoseStack pose, MultiBufferSource buf, int light) {
		...
		pose.mulPose(Vector3f.XP.rotationDegrees(-addX));

		if (this.entity == null) {
			this.entity = EntityType.TRIDENT.create(entity.getLevel());
		}

		this.eRender.render(this.entity, 0D, 0D, 0D, 0F, 0, pose, buf, light);

		if (entity.getCharge()) {
			pose.scale(2F, 2F, 2F);
			float angle = entity.tickCount / 20F * 180F / (float) Math.PI;
			pose.mulPose(Vector3f.ZP.rotationDegrees(-angle * 1.5F));
			pose.translate(-0.5D, 0.5D, -0.25D);
			pose.mulPose(Vector3f.XP.rotationDegrees(90F));
			Block SQUARE5_B = BlockInit.magic_square_l;
			float rgb2 = (float) Math.sin(entity.tickCount / 15F) * -100F;
			RenderUtil.renderBlock(pose, buf, new RenderColor(1F, (222F + rgb2) / 255F, 110F / 255F, light, OverlayTexture.NO_OVERLAY), SQUARE5_B);
		}
