...
public class SweetCrops_DoublePlant extends SweetCrops_STAGE4 {
	...
	public void randomTick(BlockState state, ServerLevel world, BlockPos pos, RandomSource rand) {
		...
		if (!this.isMaxAge(state) && ForgeHooks.onCropsGrowPre(world, pos, state, this.isGlowChange(rand))) {
			...
			if (nowAge + 1 >= 2 && world.getBlockState(pos.above()).isAir() && this.checkPlace(world.getBlockState(pos.below()), world, pos.below())) {
	...
	public void growCrops(Level world, BlockPos pos, BlockState state) {
		...
		if (age >= 2 && world.getBlockState(pos.above()).isAir() && this.checkPlace(world.getBlockState(pos.below()), world, pos.below())) {
			world.setBlock(pos.above(), this.defaultBlockState().setValue(this.getSMMaxAge(), 1), 2);
		}
	}
	...
	public void onRicghtClick(Level world, Player player, BlockState state, BlockPos pos, ItemStack stack) {
		...
		// 二段作物ならアイテムドロップ
		if (block instanceof SweetCrops_DoublePlant crop) {
	...
	public boolean canSurvive(BlockState state, LevelReader world, BlockPos pos) {
		...
		return this.mayPlaceOn(world.getBlockState(underPos1), world, underPos1) || (world.getBlockState(pos.above()).isAir() && underState1.getBlock() == this && this.mayPlaceOn(underState2, world, underPos2));
	}
