...
public class RenderLightningBolt extends EntityRenderer<ElectricMagicShot> {

	public RenderLightningBolt(EntityRendererProvider.Context con) {
		super(con);
	}

	public void render(ElectricMagicShot entity, float par1, float par2, PoseStack pose, MultiBufferSource buf, int par3) {
		...
		float f = 0F;
		float f1 = 0F;
		RandomSource rand = RandomSource.create(entity.getId() + (entity.getTick() / 2));
		...
		for (int j = 0; j < 4; ++j) {
			...
			RandomSource rand1 = RandomSource.create(entity.getId() + (entity.getTick() / 2));
	...
	public ResourceLocation getTextureLocation(ElectricMagicShot entity) {
