			...
			for (Monster entity : entityList) {
				...
				if (world instanceof ServerLevel sever) {
					...
					for (int i = 0; i < 16; ++i) {
						...
						sever.sendParticles(ParticleTypes.END_ROD, pos.getX() + this.getRand(rand, 0.5D), pos.getY() - 0.25F + this.getRand(rand, 0.25D), pos.getZ() + this.getRand(rand, 0.5D), 0, d0, d1, d2, 1F);
					}
				}
			}
		}
